...
for file_path in $(find "$RESULTS_PATH" -maxdepth 1 -type f -name "self_test-*.log") ; do
	...
	of="$CWD/dat/self_test-inflight_rpc/self_test-inflight_rpc-$servers_nb.dat"
	if [[ ${is_file_path_initialized[$of]} != true ]] ; then
		> "$of"
		is_file_path_initialized[$of]=true
	fi
	echo "$inflight_rpc;$iops" >> "$CWD/dat/self_test-inflight_rpc/self_test-inflight_rpc-$servers_nb.dat"
...
for filepath in $(find "$CWD/dat/self_test-servers_nb" "$CWD/dat/self_test-inflight_rpc" -type f -name "self_test-*.dat") ; do
...
# Generating self_test inflight_rpc graph files

if [[ $(find "$CWD//dat/self_test-inflight_rpc" -type f -name "self_test-inflight_rpc-*.dat" | wc -l) -gt 0 ]] ; then
	echo "Generaring self_test inflight_rpc graph files"
	gpi_file="$CWD/gpi/self_test-inflight_rpc/self_test-inflight_rpc.gpi"
	png_file="$CWD/png/self_test-inflight_rpc/self_test-inflight_rpc.png"
	cat > "$gpi_file" <<- EOF
	set border 11
	set xtics nomirror
	set ytics nomirror
	...
	plot \\
	EOF
	for filepath in $(find "$CWD/dat/self_test-inflight_rpc" -name "self_test-inflight_rpc-*.dat") ; do
		filename=$(basename "$filepath" .dat)
		servers_nb=$(cut -d- -f3 <<< $filename)
		cat >> $gpi_file <<- EOF
		'$filepath' using 1:2 with lines axes x1y1 title 'servers_nb=$servers_nb', \\
		EOF
	done
