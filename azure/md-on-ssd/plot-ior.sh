...
for filepath in $(find "$LOG_DIR_ROOT" \( -type d -name "bkp" -prune \) -o \( -type f -name "ior*.log" -print \)) ; do
	...
	daos_version=$(awk -F/ '{ print $(NF-7) }' <<< "$filepath")
	...
	dat_dir="$CWD/dat/md-on-ssd/ior"
	mkdir -p "$dat_dir"
	dat_file="$dat_dir/ior-$ior_mode-$ior_file_sharing-$daos_rd_fac-$daos_replication-$daos_dir_oclass-$daos_oclass-$daos_bdev_cfg-$daos_version.dat"
	...
	key="$ior_mode:$ior_file_sharing:$daos_replication:$daos_dir_oclass:$daos_oclass:$daos_bdev_cfg:$daos_version"
	if [[ -z "${max_read[$key]}" ]] ; then
		max_read[$key]="0"
		max_write[$key]="0"
	fi
	if (( $(bc -l <<< "${max_read[$key]} < $read_bw") )) ; then
		max_read[$key]=$read_bw
	fi
	if (( $(bc -l <<< "${max_write[$key]} < $write_bw") )) ; then
		max_write[$key]=$write_bw
	fi
done

for filepath in $(find $CWD/dat/md-on-ssd/ior -type f -name "*.dat") ; do
...
mkdir -p "$CWD/gpi/md-on-ssd/ior" "$CWD/png/md-on-ssd/ior"
...
for operation in read write ; do
	...
	for mode in hard ; do
		...
		for file_sharing in shared ; do
			# for daos_replication in no_replication erasure_code replication ; do
			for daos_replication in no_replication ; do
				for daos_dir_oclass in SX ; do
					for daos_oclass in SX ; do
						...
						gpi_file="$CWD/gpi/md-on-ssd/ior/ior-$mode-$file_sharing-$rd_fac-$daos_replication-$operation-$daos_dir_oclass-$daos_oclass.gpi"
						png_file="$CWD/png/md-on-ssd/ior/ior-$mode-$file_sharing-$rd_fac-$daos_replication-$operation-$daos_dir_oclass-$daos_oclass.png"
						...
						set title "IOR: io_op=$operation, mode=$mode, file_sharing=$file_sharing, redundancy=$daos_replication, daos_dir_oclass=$daos_dir_oclass, daos_oclass=$daos_oclass
						set output "$png_file"
						plot \\
						EOF
						for filepath in $(find $CWD/dat/md-on-ssd/ior -type f -name "ior-$mode-$file_sharing-$rd_fac-$daos_replication-*.dat") ; do
							filename=$(basename $filepath .dat)
							bdev_cfg=$(cut -d- -f8 <<< $filename)
							daos_version=$(cut -d- -f10 <<< $filename)
							cat >> $gpi_file <<- EOF
							'$filepath' using 1:$index with lines axes x1y1 title 'daos_version=$daos_version, bdev_cfg=$bdev_cfg', \\
							EOF
						done
...
echo "# Read"
{
	echo "mode:file sharing:redundancy:dir_oclass:oclass:bdev_config:daos_version:bandwidth"
